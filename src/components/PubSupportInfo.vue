...
<van-form @submit="onSubmitSupportInfo">
    ...
    <van-collapse v-model="activeMore">
      <van-collapse-item title="基本信息" name="basicParam"   title-class="collapseTitle">
        ...
        <van-row class="myborder">
            <van-col span='24'>
                <van-field v-model="pubTreeModel.Address" rows="2" autosize label="供货地址" type="textarea"  placeholder="供货地址" show-word-limit/> 
            </van-col>
        </van-row>
       <van-row class="myborder">
           <van-col span="6">  
                <van-cell  style="padding-left:5px;padding-right:5px;" value="热门推荐"/>
            </van-col>
             <van-col span="4">  
                 <van-cell-group>
                    <van-switch v-model="pubTreeModel.Prop3" size="30px" style="margin: 7px 1px 7px 1px" />
                </van-cell-group>      
            </van-col>
            <van-col span="14" v-show="pubTreeModel.Prop3!=0">  
                <van-field readonly clickable name="picker" :value="pubTreeModel.Prop13"  label="热门天数"  type="number"  placeholder="选择推荐天数" @click="showTisnshuPicker = true"/>
                   <van-popup v-model="showTisnshuPicker" position="bottom">
                    <van-picker show-toolbar :columns="TScolumns"  @confirm="onTSConfirm"  @cancel="showTisnshuPicker = false" />
                </van-popup>
            </van-col>
        </van-row> 
...
import { PushPic,DelPic,PushPic2,InitPubPickdata,InitPubDropDowndata,PubProduct,InitDropDownListInPub,InitPickListInPub} from '../server/api/index'
...
export default {
 ...
 InitDropdown(){
    ...
    onSubmitSupportInfo() {
        const axiosAjax = axios.create({timeout: 1000 * 60 });  
       axiosAjax.post(PubProduct,this.pubTreeModel).then();
    },
    onJGConfirm(value) {
